                ...
                <div className='space-y-3'>
                  {order.items.map((item) => (
                    <div key={item.id} className='p-3 bg-muted/50 rounded-lg space-y-2'>
                      ...
                      <div className='flex justify-between items-start gap-3'>
                        <div className='flex-1 min-w-0'>
                          ...
                          <div className='text-sm text-muted-foreground mt-0.5'>
                            {item.lineitem_quantity} × {order.currency} {item.formatted_price}
                          </div>
                          {item.lineitem_sku && (
                        ...
                        <div className='text-right shrink-0'>
                          <div className='font-semibold'>{order.currency} {item.total_price.toFixed(2)}</div>
                        </div>
                      </div>

                      {/* Linked inventory product info card */}
                      {item.client_product && (
                        <div className='flex items-center justify-between gap-2 rounded-md border border-primary/20 bg-primary/5 px-2.5 py-1.5 text-xs'>
                          <div className='flex items-center gap-1.5 min-w-0'>
                            <Package className='h-3.5 w-3.5 text-primary shrink-0' />
                            <span className='text-muted-foreground'>Inventory product:</span>
                            <span className='font-medium truncate'>{item.client_product.name}</span>
                            {item.client_product.sku && (
                              <span className='font-mono text-muted-foreground shrink-0'>[{item.client_product.sku}]</span>
                            )}
                          </div>
                          <a
                            href={`/clients/${order.client_id}?tab=products&highlight=${item.client_product_id}`}
                            target='_blank'
                      ...
                      {/* Linked catalogue product info card (dropshipper) */}
                      {item.product && (
                        <div className='flex items-center justify-between gap-2 rounded-md border border-blue-500/20 bg-blue-500/5 px-2.5 py-1.5 text-xs'>
                          <div className='flex items-center gap-1.5 min-w-0'>
                            <Package className='h-3.5 w-3.5 text-blue-500 shrink-0' />
                            <span className='text-muted-foreground'>Catalogue product:</span>
                            <span className='font-medium truncate'>{item.product.title}</span>
                            {item.product.variant_sku && (
                              <span className='font-mono text-muted-foreground shrink-0'>[{item.product.variant_sku}]</span>
                            )}
                          </div>
                          <a
                            href={`/inventory?search=${encodeURIComponent(item.product.variant_sku ?? item.product.title ?? '')}`}
                            target='_blank'
                            rel='noopener noreferrer'
                            className='text-blue-500 hover:underline shrink-0 font-medium'
                          >
                            View →
                          </a>
                        </div>
