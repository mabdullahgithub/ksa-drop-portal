...
export function RolesActionDialog({ availablePermissions }: RolesActionDialogProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isEdit) {
      put(route('team-management.roles.update', currentRow.id), {
        onSuccess: () => {
          setOpen(null)
          reset()
      ...
      <DialogContent className='max-w-4xl max-h-[90vh] flex flex-col p-0'>
        <form onSubmit={handleSubmit} className='flex flex-col h-full max-h-[90vh]'>
          <DialogHeader className='px-6 pt-6 pb-4 flex-shrink-0'>
            ...
            <DialogDescription>
              {isEdit
                ? 'Update the role details and assign permissions to control access levels.'
